...
const TestConnect = () => {
    ...
    return (
        <div className={"TextConnect"}>
            ...
            {/* 2. dto 응답 테스트 */}
            <GetButton text={"(단순) dto 응답 테스트"} url={"/api/dto"} api={api}/>
            {/* 3. map 응답 테스트 */}
            <GetButton text={"(단순) map 응답 테스트"} url={"/api/map"} api={api}/>

            <br/>

            {/* 4. 상태변환 String 응답 테스트 */}
            <div className={"status_request"}>
                <GetButton text={"(Ok 응답) String 응답 테스트"} url={"/api/statusTextOk"} api={api}/>
                <GetButton text={"(Bad 응답) String 응답 테스트"} url={"/api/statusTextBad"} api={api}/>
            </div>

            {/* 5. 상태변환 dto 응답 테스트 */}
            <div className={"status_request"}>
                <GetButton text={"(Ok 응답) dto 응답 테스트"} url={"/api/statusDtoOk"} api={api}/>
                <GetButton text={"(Bad 응답) dto 응답 테스트"} url={"/api/statusDtoBad"} api={api}/>
            </div>

            {/* 6. 상태변환 dto 응답 테스트 */}
            <div className={"status_request"}>
                <GetButton text={"(Ok 응답) map 응답 테스트"} url={"/api/statusMapOk"} api={api}/>
                <GetButton text={"(Bad 응답) map 응답 테스트"} url={"/api/statusMapBad"} api={api}/>
            </div>
